import { pgTable, pgEnum, serial, text, uniqueIndex, index, foreignKey, varchar, timestamp, unique, uuid, boolean, smallint } from "drizzle-orm/pg-core"

export const dimension_unit = pgEnum("dimension_unit", ['count', 'percent'])
export const event_handlers_target_enum = pgEnum("event_handlers_target_enum", ['page', 'component', 'data_query', 'table_column', 'table_action'])
export const layput_type = pgEnum("layput_type", ['desktop', 'mobile'])
export const scope = pgEnum("scope", ['local', 'global'])
export const source = pgEnum("source", ['signup', 'invite', 'google', 'git', 'workspace_signup'])
export const status = pgEnum("status", ['invited', 'verified', 'active', 'archived'])
...
export const resource = pgTable("resource", {
	...
	(table) => {
		return {
			...
			parentId_fkey: index("resource_parentId_fkey").using("btree", table.parentId),
...
export const role_permission = pgTable("role_permission", {
	id: varchar("id", { length: 191 }).primaryKey().notNull(),
	resourceId: varchar("resourceId", { length: 191 }).notNull().references(() => resource.id),
	...
	(table) => {
		return {
			resourceId_fkey: index("role_permission_resourceId_fkey").using("btree", table.resourceId),
			roleId_fkey: index("role_permission_roleId_fkey").using("btree", table.roleId),
		}
	});
...
export const role = pgTable("role", {
	...
	(table) => {
		return {
			...
			profileId_key: uniqueIndex("role_profileId_key").using("btree", table.profileId),
		}
	});
...
export const user_permissions = pgTable("user_permissions", {
	...
	(table) => {
		return {
			resourceId_fkey: index("user_permissions_resourceId_fkey").using("btree", table.resourceId),
		}
	});
...
export const user_role = pgTable("user_role", {
	...
	(table) => {
		return {
			...
			userId_key: uniqueIndex("user_role_userId_key").using("btree", table.userId),
		}
	});
...
export const organizations = pgTable("organizations", {
	...
	(table) => {
		return {
			name_organizations_unique: unique("name_organizations_unique").on(table.name),
			slug_organizations_unique: unique("slug_organizations_unique").on(table.slug),
		}
	});
